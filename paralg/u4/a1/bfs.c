...
static counter_t bfs_sequential(
		graph_t g,	       // I: connected undirected graph
...
static counter_t bfs_parallel(
		graph_t g,	       // I: connected undirected graph
		vertex_t source,	       // I: source vertex
		counter_t bfs_level[]    // O: array with BFS level for each vertex
	) {
	counter_t n_vertices = graphGetNVertices(g);
	counter_t max_level = 0;
	...
	#pragma omp parallel
	{
		...
		graphVertexIterator(g, vi) {
			vertex_t v = graphVertexIteratorItem(g, vi);
			bfs_level[v] = UNLIMITED;
		}
		#pragma omp single
		bfs_level[source] = 0;
