...
for aspect in aspects.items():
    ...
    for problem in problems:
        ...
        for aspect_option in aspect_options:
            ...
            if len(values) > 0:
                cycles  = (values[0] - values[1]) * 3 # L1 hits
                cycles += (values[2] - values[3]) * 17  + (values[1] * linesfactor * line_penalty[0]) # L2 hits
                cycles += (values[4] - values[5]) * 49  + (values[3] * linesfactor * line_penalty[1]) # L3 hits
                cycles += values[5]               * 310 + (values[5] * linesfactor * line_penalty[2]) # main memory
